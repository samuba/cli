...
var (
	restrictionsCmd = &cobra.Command{
		...
		Short:   "Manage network restrictions for Supabase projects",
	...
	restrictionsUpdateCmd = &cobra.Command{
		...
		Long:  `Updates the network restrictions on a Supabase project.`,
	...
	restrictionsGetCmd = &cobra.Command{
		Use:   "get",
		Short: "Get the current network restrictions for a Supabase project.",
		RunE: func(cmd *cobra.Command, args []string) error {
			fsys := afero.NewOsFs()
			if err := PromptLogin(fsys); err != nil {
				return err
